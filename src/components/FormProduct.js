...
function validate(infoFormData)
{
  console.log(infoFormData.get('title'), 'data en el validate');
    if(infoFormData.get('title').length== 0)
    {
      setTitulo(true)
    }
    if(infoFormData.get('price').length ==0)
    {
      setPrice(true)
    }
    if(infoFormData.get('description').length==0)
    {
      setDescripcion(true)
    }
    if(infoFormData.get('category').length ==0)
    {
      setCategory(true)
    }

    if(titulo && price && descripcion && category)
    {
      return true
    }
    else{
